...
void UpdateCombine(Combine* c) {
	PORTB = c->blink->state | c->three->state;
	}
*/
int main(void)
{
	...
	three->tick = 1000;
	three->state = 0;
	three->Update = UpdateThree;
	Blink* blink;
	blink->tick = 1000;
	blink->state = 0;
	blink->Update = UpdateBlink;
	Combine* combine;
	combine->blink = &blink;
	combine->three = &three;
	combine->Update = UpdateCombine;
